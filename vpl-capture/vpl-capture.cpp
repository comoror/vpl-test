// vpl-capture.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//

#include <iostream>
#include <vpl/mfx.h>
#include "util-vpl/defs.h"
#include "util-vpl/util.h"
#include "util-vpl/vpl-timing.h"
#include "util-vpl/hw-device.h"
#include "util-vpl/capture-frames.h"
...
static int ProcessStreamEncode(mfxSession session, FrameInfo* frameInfo, FileInfo* fileInfo, EncodeCtx* encCtx, mfxU32 dbgMask) {
    ...
    while (bIsStillGoing == true) {
        ...
        // encode next frame
        mfxSyncPoint syncp = {};
        //set mfxEncodeCtrl for keyframe insertion
        mfxEncodeCtrl ctrl = {};
        ...
        case MFX_ERR_MORE_DATA:
            if (bIsDrainingEnc == true)
                bIsStillGoing = false;
            std::cout << "INFO: MFX_ERR_MORE_DATA\n";
            break;
...
int main() {
    ...
    fileInfo->outfile.open(params->outfileName, std::ios::binary);
    ...
    if (params->testMode == TEST_MODE_CAPTURE) {
        ...
        if ((params->surfaceMode == SURFACE_MODE_SHARED) && ((w != ALIGN16(w)) || (h != ALIGN16(h)))) {
    ...
    fileInfo->bitstream.CodecId = MFX_CODEC_AV1;// MFX_CODEC_HEVC;

    // init encode parameters
    mfxVideoParam mfxEncParams = {};
    mfxEncParams.mfx.CodecId = MFX_CODEC_AV1;//MFX_CODEC_HEVC;
